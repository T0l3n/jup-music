	...
	run: async (client, message, args) => {
		try {
			...
			let newMsg = await message.reply({
				content: `${client.allEmojis.loading} Cargando el **'${args[0] ? args[0] : "Default"}' Music Mix**`,
			});
			try {
				...
				//Edit the reply
				newMsg.edit({
					content: `${queue?.songs?.length > 0 ? "👍 Cargado" : "🎶 Now Playing"}: the **'${args[0] ? args[0] : "Default"}'**`,
				});
			} catch (e) {
