		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Agregue una duración de la posición del número de búsqueda!**`)
				...
				if (seekNumber > newQueue.songs[0].duration || seekNumber < 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **La posición de búsqueda debe estar entre \`0\` y \`${newQueue.songs[0].duration}\`!**`)
					],
