...
module.exports = {
	...
	description: "Habilitar/deshabilitar el bucle de canción/cola", //the command description for Slash Command Overview
	cooldown: 5,
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Agregue opciones válidas!**`)
				...
				if (newQueue.repeatMode == 0) {
					message.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`${client.allEmojis.x} **Deshabilitado el Loop!**`)
