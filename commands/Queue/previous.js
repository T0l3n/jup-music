...
module.exports = {
	...
	description: "Reproduce la canción anterior!", //the command description for Slash Command Overview
	cooldown: 10,
		...
		try {
			...
			try {
				...
				if (!newQueue || !newQueue.previousSongs || newQueue.previousSongs.length == 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **No hay canciones anteriores!**`)
					],
