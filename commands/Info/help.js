      ...
      if (args[0] && args[0].length > 0) {
        ...
        if (!cmd) {
          return message.reply({
            embeds: [embed.setColor(ee.wrongcolor).setDescription(`No se encontr贸 informaci贸n para el comando **${args.toLowerCase()}**`)]
          });
        }
        if (cmd.name) embed.addField("**Nombre del comando**", `\`${cmd.name}\``);
        if (cmd.name) embed.setTitle(`Informaci贸n detallada sobre:\`${cmd.name}\``);
        ...
        const embed = new MessageEmbed()
          ...
          .setTitle("HELP MENU 馃敯 Commands")
          ...
          .setFooter(`Para ver la descripci贸n y la informaci贸n del comando, use: ${prefix}help [CMD NAME]`, ee.footericon);
