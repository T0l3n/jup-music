...
module.exports = {
	...
	description: "Reproduce una canción/lista de reproducción y la agrega a la parte superior!", //the command description for Slash Command Overview
	cooldown: 2,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		...
		//{"Integer": { name: "ping_amount", description: "How many times do you want to ping?", required: true }}, //to use in the code: interacton.getInteger("ping_amount")
		{
			"String": {
				name: "song",
				description: "Que cancion quieres tocar",
				required: true
			}
		...
		try {
			...
			if (channel.userLimit != 0 && channel.full)
				return interaction.reply({
					embeds: [new MessageEmbed()
						...
						.setTitle(`<:declined:780403017160982538> El canal de voz está lleno, no puedo unirme!`)
					],
					ephemeral: true
				});
			if (channel.guild.me.voice.channel && channel.guild.me.voice.channel.id != channel.id) {
				return interaction.reply({
					embeds: [new MessageEmbed()
						.setColor(ee.wrongcolor)
						.setFooter(ee.footertext, ee.footericon)
						.setTitle(`<:declined:780403017160982538> Ya estoy conectado en otro canal`)
					],
					ephemeral: true
				});
			}
			...
			try {
				...
				interaction.editReply({
					content: `${queue?.songs?.length > 0 ? "👍 Añadido al principio de la cola" : "🎶 Now Playing"}: \`\`\`css\n${Text}\n\`\`\``,
					ephemeral: true
				});
			} catch (e) {
