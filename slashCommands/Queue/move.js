...
module.exports = {
	...
	description: "Mueve una canción a otro lugar", //the command description for Slash Command Overview
	cooldown: 10,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		...
		}, //to use in the code: interacton.getInteger("ping_amount")
		{
			"Integer": {
				name: "to_where",
				description: "¿A dónde debo mover la canción? (1 == after current, -1 == Top)",
				required: true
			}
		}, //to use in the code: interacton.getInteger("ping_amount")
			...
			try {
				...
				if (songIndex > newQueue.songs.length - 1) return interaction.reply({
					embeds: [
						...
						.setDescription(`**La última canción en la cola tiene el índice: \`${newQueue.songs.length}\`**`)
					],
					ephemeral: true
				})
				if (position == 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **No se puede mover la canción antes de reproducirla!**`)
					],
					ephemeral: true
				})
