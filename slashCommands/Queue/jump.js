...
module.exports = {
	...
	description: "Salta a una canción específica en la colae", //the command description for Slash Command Overview
	cooldown: 10,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		{
			"Integer": {
				name: "position",
				description: "¿A qué canción quieres saltar en la cola?",
				required: true
			}
		...
		try {
			...
			try {
				...
				if (Position > newQueue.songs.length - 1 || Position < 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **La posición debe estar entre \`0\` y \`${newQueue.songs.length - 1}\`!**`)
					],
					ephemeral: true
				})
