...
module.exports = {
	...
	description: "Elimina una o más canciones", //the command description for Slash Command Overview
	cooldown: 10,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		//INFORMATIONS! You can add Options, but mind that the NAME MUST BE LOWERCASED! AND NO SPACES!!!, for the CHOCIES you need to add a array of arrays; [ ["",""] , ["",""] ] 
		{
			"Integer": {
				name: "what_song",
				description: "¿Qué índice de canciones quieres eliminar?",
				required: true
			}
		...
		//{"String": { name: "song", description: "Que cancion quieres tocar", required: true }}, //to use in the code: interacton.getString("title")
		...
		try {
			...
			try {
				...
				if (songIndex <= 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **No puedes eliminar la canción actual (0)!**`)
						.setDescription(`**Utilize el \`${client.settings.get(guild.id, "prefix")}skip\` (Slash)Command instead!**`)
					],
					ephemeral: true
				})
				if (amount <= 0) return interaction.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setTitle(`${client.allEmojis.x} **Necesitas eliminar al menos 1 canción!**`)
					],
					ephemeral: true
				})
