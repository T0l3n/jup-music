...
module.exports = {
	...
	description: "Habilitar/deshabilitar el loop de canción/cola", //the command description for Slash Command Overview
	cooldown: 5,
	...
	options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!
		...
		//{"Role": { name: "what_role", description: "To Ping a Role lol", required: false }}, //to use in the code: interacton.getRole("what_role")
		{
			"StringChoices": {
				name: "what_loop",
				description: "¿Qué loop quieres configurar??",
				required: true,
				choices: [
					["Disable", "0"],
					["Song Loop", "1"],
				...
				if (newQueue.repeatMode == 0) {
					interaction.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`${client.allEmojis.x} **Deshabilitado el modo Loop!**`)
					...
					interaction.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`🔁 **Enabled the __Song__-Loop** ||(Deshabilitado **Queue-Loop**)||`)
						  .setFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true}))]
						})
				} else {
					interaction.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setTitle(`🔂 **Enabled the __Queue__-Loop!** ||(Inhabilitado el ** Song-Loop**)||`)
						  .setFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true}))]
						})
				}
			} catch (e) {
