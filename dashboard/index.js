...
module.exports = client => {
    ...
    app.get(`/callback`, passport.authenticate(`discord`, { failureRedirect: "/" }), async (req, res) => {
        ...
        if(banned) {
                req.session.destroy(() => {
                res.json({ login: false, message: `Has sido bloqueado del Dashboard.`, logout: true })
                req.logout();
            });
        } else {
    ...
    app.get("/dashboard", checkAuth, async (req,res) => {
      ...
      return res.redirect("/?error=" + encodeURIComponent("Inicie sesión primero!"));
      if(!req.user.guilds)
      return res.redirect("/?error=" + encodeURIComponent("No se pueden obtener tus guilds!"));
    ...
    app.get("/dashboard/:guildID", checkAuth, async (req, res) => {
      ...
      if (!guild) return res.redirect("/dashboard?error=" + encodeURIComponent("No se pueden obtener los datos de información del guild"));
      ...
      if (!member) return res.redirect("/dashboard?error=" + encodeURIComponent("No pudimos encontrarte, lo siento.!"));
      if (!member.permissions.has(Permissions.FLAGS.MANAGE_GUILD)) {
        return res.redirect("/dashboard?error=" + encodeURIComponent("No tienes permitido hacer eso!"));
      }
    ...
    app.post("/dashboard/:guildID", checkAuth, async (req, res) => {
      ...
      if (!member) return res.redirect("/dashboard?error=" + encodeURIComponent("No puedo Información Datos sobre usted!"));
      if (!member.permissions.has("MANAGE_GUILD")) {
        return res.redirect("/dashboard?error=" + encodeURIComponent("No tienes permitido hacer esot!"));
      }
    ...
    app.get("/queuedashboard", checkAuth, async (req,res) => {
      if(!req.isAuthenticated() || !req.user) 
      return res.redirect("/?error=" + encodeURIComponent("Inicie sesión primerot!"));
      if(!req.user.guilds)
      return res.redirect("/?error=" + encodeURIComponent("No se pueden obtener tus guild!"));
